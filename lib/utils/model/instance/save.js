...
module.exports = function saveModelInstance(values, queryOptions) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (this.debug.enabled) {
      this.debug('save(%s)', utils.model.helpers.debugInspect(values));
    }
    ...
    async.waterfall([
      ...
      this.Class.callback(utils.model.events.beforeEvent, 'beforeSave', values, queryOptions),
      this.callback(driverUtil.save, values, queryOptions),
      this.Class.callback(utils.model.events.afterEvent, 'afterSave', values, queryOptions)
    ],
    this.callback(utils.model.helpers.handleResult.updateReferencedModel, resolve, reject));
  }
  .bind(this));
};