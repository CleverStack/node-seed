...
function testEnv( cb ) {
    var ormEnabled = packageJson.bundledDependencies.indexOf( 'orm' ) !== -1
      , odmEnabled = packageJson.bundledDependencies.indexOf( 'odm' ) !== -1
    ...
    if ( ormEnabled ) {
        ...
        injector.getInstance( 'sequelize' )
            ...
            .error(function( err ) {
                debug( 'Error trying to connect to ' + env.config.orm.db.options.dialect, err );
            });
    }
}
