...
module.exports = {
  ...
  loadModulesFileByName: function(fileName, config, cluster, debug) {
    packageJson.bundledDependencies.forEach(function(moduleName) {
      var file = path.resolve(path.join(__dirname, '..', '..', 'modules', moduleName, 'bin', fileName));
  ...
  supportSingleDbModule: function(env, dbModuleName, moduleName) {
    if (moduleName) {
      env.packageJson.bundledDependencies.length = 0;
      env.packageJson.bundledDependencies.push(dbModuleName, env);
