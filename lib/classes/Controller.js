...
module.exports = require( 'clever-controller' ).extend(
/* @Static */
{
...
/* @Prototype */
{
    getOptionsForService: function() {
        var options = {
            where: underscore.omit( this.req.query, '_include', '_limit', '_offset' ),
        };
    ...
    putAction: function() {
        if ( !this.req.body.id && !this.req.params.id && !this.req.query.id ) {
            ...
            return this.Class.service.update( options, underscore.omit(this.req.body, 'id', 'createdAt', 'updatedAt') );
        } else {
            this.next();
        }
    },
