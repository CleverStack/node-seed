...
},
{
    ...
    addFolderToPath: function( injector, folder ) {
        ...
        if ( rootFolder !== 'models' || ( rootFolder === 'models' && moduleLoader.moduleIsEnabled( 'clever-' + currentFolder ) ) ) {
            this.paths.push( p );
    ...
    addResource: function( pathToInspect, file ) {
        if ( file.match(/.+\.js$/g) !== null && this.Class.excludedFiles.indexOf( file ) === -1 ) {
            ...
            if ( rootFolder === 'models' ) {
                ...
                if ( moduleLoader.moduleIsEnabled( 'clever-' + currentFolder ) ) {
                    lastFolder[ name ] = require( 'clever-' + currentFolder ).getModel( [ pathToInspect, '/', file ].join( '' ) );
                }
            } else {
                // Load the resource
