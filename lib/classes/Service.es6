...
import {Class, Model} from 'classes';
...
var debug = debugLog('cleverstack:services');
...
export default class Service extends Class {
  ...
  query(sql, raw = false) {
    return this.db.query(sql, null, {raw: raw});
  }
  ...
  transaction(queryOptions = {}) {
    return new Promise((resolve, reject) => {
      if (queryOptions.transaction) {
        ...
        this.db.transaction({ autocommit: false }).then(function(transaction) {
          if (transaction) {
  ...
  create(values = {}, queryOptions = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (!this.model) {
        return reject(new Error(`Model not found, either set ${this.name}.model or implement ${this.name}.create()`));
      }

      if (!!values.id) {
        return reject(new exceptions.DuplicateModel(`Unable to create a new ${this.model.name}, identity already exists.`));
      }
  ...
  find(idOrFindOptions, queryOptions = {}) {
    ...
    return new Promise((resolve, reject) => {
      if (!this.model) {
        return reject(`Model not found, either set ${this.name}.model or implement ${this.name}.find()`);
      }
  ...
  findAndUpdate(values, queryOptions = {}) {
    if (!queryOptions.where) {
      queryOptions.where = {
        id: queryOptions
      };
