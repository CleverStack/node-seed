...
let i           = inflect()
  ...
  , moduleDebug = debug('cleverstack:modules')
  , moduleLdr   = injector.getInstance('moduleLoader')
  , modules     = {};

export default class Module extends Class {
  static moduleFolders = [
    'exceptions',
    'classes',
    'models',
    'services',
  ...
  constructor() {
    ...
    if (Reg.test(stack[ 2 ])) {
      this.path  = RegExp.$1.split(path.sep);
      this.path  = this.path.splice(0, this.path.length - 1).join(path.sep);
      pkg        = path.resolve(path.join(this.path, 'package.json'));
    ...
    this._camelName = i.camelize(this.name.replace(/\-/ig, '_'), false);
    moduleDebug(`Creating debugger with name ${this._camelName}...`);
    this.debug = require('debug')(`cleverstack: ${this._camelName}`);

    // Set our config if there is any
    this.config = typeof config[this.name] === 'object' ? config[this.name] : {};

    // Ensure its dependencies are enabled and will be loaded
    if (this.pkg && this.pkg.peerDependencies) {
      Object.keys(this.pkg.peerDependencies).forEach(this.proxy(function(dependency) {
    ...
    if (moduleLdr.moduleIsEnabled('clever-background-tasks')) {
      Module.moduleFolders.push('tasks');
    }

    // Add our moduleFolders to the list of paths, and our injector paths
    Module.moduleFolders.forEach(this.proxy('addFolderToPath', injector));
   ...
   * @return {undefined}
   */
  inspectPathForResources(pathToInspect, callback) {
    var that = this;
   ...
   * @return {undefined}
   */
  addResource(pathToInspect, file, callback) {
    if ((file.match(/.+\.js$/g) !== null || file.match(/.+\.es6$/g) !== null) && Module.excludedFiles.indexOf(file) === -1) {
      ...
      if (Module.injectableFolders.indexOf(rootFolder) === -1) {
        
        if (Module.injectableFolders.indexOf(name) === -1)  {
   ...
   * @return {undefined}
   */
  addFolderToPath(injector, folder) {
